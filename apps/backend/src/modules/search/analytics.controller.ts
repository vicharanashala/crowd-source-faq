...
function parseAndValidateBatchId(req: Request, res: Response): string | null | undefined {
  let batchIdParam = req.query.batchId;
  
  if (batchIdParam === undefined) {
    return null;
  }

  if (Array.isArray(batchIdParam)) {
    batchIdParam = batchIdParam[0];
  }

  if (typeof batchIdParam === 'string' && batchIdParam.includes(',')) {
    batchIdParam = batchIdParam.split(',')[0];
  }
...
export const getSearchAnalytics = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const popularQueries: PopularQuery[] = await SearchLog.aggregate([
      ...(batchIdObj ? [{ $match: { batchId: new mongoose.Types.ObjectId(batchIdObj) } }] : []),
      {
        $group: {
          _id: { $toLower: '$query' },
          count: { $sum: 1 },
