...
export const generateEmbedding = async (text: string, options?: { batchId?: string | null }): Promise<number[]> => {
  ...
  if (redisClient) {
    try {
      ...
      logger.warn(`[embeddings] Redis cache get failed: ${(err as Error).message}`);
    }
  }

  try {
    const { model, baseURL, apiKey } = await getActiveEmbeddingConfig(options?.batchId);
    const vec = await callCustomEmbedding(text, apiKey, model, baseURL);

    if (redisClient && Array.isArray(vec) && vec.length === dimensions) {
      try {
        await redisClient.set(cacheKey, JSON.stringify(vec), 'EX', 604800); // 7 days TTL
      } catch (err) {
        logger.warn(`[embeddings] Redis cache set failed: ${(err as Error).message}`);
      }
    }
    return vec;
  } catch (err) {
