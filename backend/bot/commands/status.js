...
function statusActions(portalUrl) {
    const portal = portalUrl.replace(/\/+$/, '');
    return [
        new ActionRowBuilder().addComponents(new ButtonBuilder()
            .setStyle(ButtonStyle.Link)
            .setLabel('🔍 Search the portal')
            .setURL(`${portal}/`), new ButtonBuilder()
            .setStyle(ButtonStyle.Link)
            .setLabel('❓ Top unanswered')
            .setURL(`${portal}/unresolved`), new ButtonBuilder()
            .setStyle(ButtonStyle.Link)
            .setLabel('📋 My stats')
...
export async function executeStatus(interaction, config, batchId = null) {
    ...
    try {
        ...
        await interaction.followUp({
            embeds: [errorEmbed(`Couldn't fetch health: ${err.message}`)],
        });
        return;
    }
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: '📚 FAQs', value: `${data.faqs ?? '?'}`, inline: true }, { name: '💬 Community', value: `${data.posts ?? '?'}`, inline: true }, { name: '🔎 Searches today', value: `${data.searchesToday ?? '?'}`, inline: true }, {
        name: '🛟 Support',
        value: [
            `**open**     ${s.open ?? '?'}`,
            `**pending**  ${s.pending ?? '?'}`,
        ...
        inline: false,
    }, { name: '❓ Unanswered', value: `${data.unanswered ?? '?'} pending FAQ(s)`, inline: true }, { name: '🏷️ Top category', value: `\`${data.topCategory ?? 'N/A'}\``, inline: true })
