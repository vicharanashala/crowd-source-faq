...
export async function banAndRejectGoldenTicket(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    request.statusHistory.push({
      status: 'Rejected',
      note: reason
        ? `${reason} | Penalty: -${penalty} SP. 72h ban applied (until ${user.goldenBannedUntil.toISOString()}).`
        : `Banned + rejected by ${auth.name}. Penalty: -${penalty} SP. 72h ban applied (until ${user.goldenBannedUntil.toISOString()}).`,
      updatedBy: auth.userId,
      updatedByName: auth.name,
      timestamp: now,
    });
    ...
    await logAdminAction(
      ...
      request._id,
      `Ban+Reject Golden ticket. Penalty: -${penalty} SP (1.25x of ${request.spCost}). 72h ban until ${user.goldenBannedUntil.toISOString()}.${reason ? ` | ${reason.slice(0, 120)}` : ''}`,
    );

    await notifyUser(request.userId, {
      title: 'Your Golden ticket was rejected — 72h restriction applied',
      message: `An admin rejected your Golden ticket and applied a 72-hour content-creation restriction (until ${user.goldenBannedUntil.toISOString()}). You can still browse the platform.`,
      link: '/support/' + request._id.toString(),
      metadata: {
        supportRequestId: request._id.toString(),
        status: 'Rejected',
