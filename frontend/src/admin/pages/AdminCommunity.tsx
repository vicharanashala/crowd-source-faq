...
export default function AdminCommunity() {
  ...
  return (
    <div className="space-y-4 max-w-6xl">
      ...
      <div><h2 className="text-lg font-semibold text-admin-text">Community</h2><p className="text-sm text-admin-muted mt-0.5">{total} total posts</p></div>
      ...
      <div className="bg-admin-card border border-white/5 rounded-lg overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? <tr><td colSpan={8} className="px-3 py-6"><TableSkeleton rows={8} /></td></tr> :
               posts.length === 0 ? <tr><td colSpan={8} className="px-3 py-10 text-center text-sm text-admin-muted">No posts found</td></tr> :
               posts.map(post => (
                <tr key={post._id} className="border-b border-white/5 last:border-0 hover:bg-admin-bg transition-colors">
                  <td className={`${td} max-w-[180px] truncate`} title={post.title}>{post.title}</td>
                  <td className={`${td} text-admin-muted`}>{post.author?.name ?? '—'}</td>
                  ...
                  <td className={`${td} text-right text-admin-muted tabular-nums`}>{post.comments?.length ?? 0}</td>
                  <td className={`${td} text-right text-admin-muted tabular-nums`}>{post.upvotes?.length ?? 0}</td>
                  <td className={`${td} text-admin-muted`}>{new Date(post.createdAt).toLocaleDateString('en-IN')}</td>
                  <td className={`${td} text-right`}>
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => setViewPost(post)} className="w-6 h-6 flex items-center justify-center rounded text-admin-muted hover:text-admin-text hover:bg-admin-surface transition-colors" title="View"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
                      <button onClick={() => handleDelete(post._id)} className="w-6 h-6 flex items-center justify-center rounded text-admin-muted hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {pages > 1 && <div className="flex items-center justify-between px-4 py-3 border-t border-white/5 text-sm text-admin-muted"><span>Page {page} of {pages} · {total} results</span><div className="flex gap-1"><button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1 rounded-md hover:bg-admin-surface disabled:opacity-30 transition-colors">← Prev</button><button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages} className="px-3 py-1 rounded-md hover:bg-admin-surface disabled:opacity-30 transition-colors">Next →</button></div></div>}
      </div>

      <Modal open={!!viewPost} onClose={() => setViewPost(null)} title="Post Details">
        {viewPost && (
          <div className="space-y-3">
            <div><p className="text-xs font-medium text-admin-muted mb-1">Title</p><p className="text-sm text-admin-text">{viewPost.title}</p></div>
            <div><p className="text-xs font-medium text-admin-muted mb-1">Author</p><p className="text-sm text-admin-text">{viewPost.author?.name} ({viewPost.author?.email})</p></div>
            <div><p className="text-xs font-medium text-admin-muted mb-1">Body</p><p className="text-sm text-admin-text whitespace-pre-wrap">{viewPost.body}</p></div>
            <div><p className="text-xs font-medium text-admin-muted mb-1">Status</p><Badge status={viewPost.status === 'answered' ? 'approved' : 'pending'} label={viewPost.status} showDot={false} /></div>
            {viewPost.answer && <div><p className="text-xs font-medium text-admin-muted mb-1">Official Answer</p><p className="text-sm text-emerald-700 whitespace-pre-wrap border-l-2 border-emerald-300 pl-3">{viewPost.answer}</p></div>}
              ...
              <div className="p-3 rounded-lg bg-red-50 border border-red-200">
                ...
                <div className="space-y-1.5 max-h-32 overflow-y-auto">
                  {viewPost.reports.map((r, i) => (
                    <div key={i} className="text-xs text-red-700 bg-admin-card rounded-lg px-3 py-2 border border-red-100">
            ...
            <div>
              <p className="text-xs font-medium text-admin-muted mb-1">Comments ({viewPost.comments?.length ?? 0})</p>
              {viewPost.comments?.length ? <div className="space-y-1.5 max-h-40 overflow-y-auto">{viewPost.comments.map(c => <div key={c._id} className="text-sm text-admin-text bg-admin-bg rounded-lg px-3 py-2 border border-white/5"><span className="font-medium">{c.author?.name ?? '—'}: </span>{c.body}{c.verified && <span className="ml-2 text-emerald-500">✓</span>}</div>)}</div> : <p className="text-xs text-admin-muted">No comments</p>}
            </div>
            <div className="flex justify-between pt-2 border-t border-white/5">
              <span className="text-xs text-admin-muted">{new Date(viewPost.createdAt).toLocaleString('en-IN')}</span>
              <div className="flex gap-2">
                ...
                <button onClick={() => setViewPost(null)} className="px-3 py-1.5 rounded-md text-xs text-admin-muted hover:text-admin-text hover:bg-admin-surface transition-colors">Close</button>
              </div>
            </div>
          </div>
        )}
