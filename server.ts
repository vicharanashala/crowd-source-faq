...
if (process.env.NODE_ENV !== 'production') {
  ...
  app.listen(PORT, async () => {
    ...
    // Retry failed Zoom meetings — every 15 minutes
    const retryInterval = setInterval(() => {
      retryFailedMeetings().catch((e: Error) => logger.error(`[Zoom Retry] ${e.message}`));
    }, 15 * 60 * 1000);
    retryFailedMeetings().catch((e: Error) => logger.error(`[Zoom Retry] Initial: ${e.message}`));
